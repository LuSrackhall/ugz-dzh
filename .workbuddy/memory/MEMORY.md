...
## 期初双源冲突采信 JSON（铁律三落地，2026-09-06 下游 bug 反馈）

- **下游 0.9.0 实测 bug**：科目被凭证冲平（期末=0）后静置一个月再重现，期初被重置回建账期初（宏远公司1 六月冲平、八月重现 1,003 万）；下游手工重建 JSON 余额链 179 处后 -f 重跑会复发。
- **根因**：GetInitBalanceForGenerate 路径 2（上月 xlsx 账页链）压过路径 3（JSON 权威链）。触发态=xlsx 链陈旧（冲平凭证从未级联重建进 xlsx）+ JSON 已手工修正 → -f 从陈旧账页链读回旧余额、重新污染 Balances。
- **修复**：双源比对——健康账本两源恒等（回写与月结同源同月）；不一致 → 采信 JSON + 告警，-f 重建自愈。仅单源时行为不变。签名 (int64, string warn)。
- **教训**：干净账本全流程复现不出（各级 -f 全对）——触发态在"外部手修 JSON + xlsx 未同步"；复现要构造的是状态（双源分歧），不是更多操作序列。
