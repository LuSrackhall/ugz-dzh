...
| 层次 | 名称 | 职责 | 管辖 | 示例 |
|---|---|---|---|---|
| 第 1 层 | **Sheet 编排层** | 正页/反页在 Excel Sheet 中的位置排布（并排/上下、装订边、列间隙）。读取第 2 层的每页总行数决定 Sheet 中共需多少正/反页位置 | LayoutEngine | "GL 左半 sheet 是正面，右半是反面" |
| 第 2 层 | **正/反页内部层** | 每张 A4 纸正页或反页内部的完整结构（标题区、表头区、数据区、页脚区），含**过次页/承前页数据流控制**和**每页数据行数** | **本宪法** | "标题深绿 #006100、14pt 加粗；每页 20 行数据后过次页，新页首行承前页" |
...
**过次页/承前页是第 2 层内部的数据流控制**——20 行数据后写"过次页"、新页首行写"承前页"、页内余额累算，这些都属于正页/反页的内部逻辑，第 1 层不感知也不参与。

第 1 层编排时只读取第 2 层的**每页总行数**（含标题行 + 表头行 + 数据行 + 过次页行 + 承前页行），用来在 Sheet 中计算正页/反页的占位行数和物理页总数，对"过次页"是什么无任何兴趣。

```
第 2 层定义：每页行数、过次页/承前页逻辑、标题样式、列宽比例
     │
     ▼ 提供每页总行数
第 1 层编排：并排/上下、装订边、列间隙，用每页总行数算 Sheet 占用
     │
     ▼ 提供 Layout
Renderer 按坐标渲染
```

第 1 层负责"数据放在 Sheet 的哪里"，第 2 层负责"放上去的时候长什么样、何时过次页"。互不依赖，通过 Layout 协作。
...
- **LayoutSpec** — 物理约束（纸张、边距、列比例）。`每页行数` 属于第 2 层参数，第 1 层只读取它。
...
- `每页行数`（DataRowsPerPage）— 一张物理页数据区容纳多少行分录
- 标题区高度、标题字体/字号/颜色
- 表头区列宽比例、列名文字样式
- 数据区行高、金额格式、字体

ComputeLayout 为第 2 层计算出页内坐标，Renderer 按坐标渲染，不自行推导样式。第 1 层从 Layout 中读取 `每页行数` 用于编排，但不修改它。
...
| 数据区（Body） | 分录行、过次页行、承前页行 | 所有页同行高、同字体、同 `每页行数`。**过次页/承前页是第 2 层内部数据流控制**，第 1 层不感知 |
| 页脚区（Footer） | 过次页合计行、承前页承接行（属于数据区首/末行） | 同数据行标准 |

- 区域拥有稳定边界（由 LayoutSpec 定义），区域之间互不依赖。
...
Step 2: 第 1 层变更 → 调整 LayoutSpec 中 Sheet 编排参数（并排/上下、装订边、列间隙）。读取第 2 层提供的**每页总行数**计算 Sheet 中正/反页位置。
...
```
Sheet 编排 → 页内样式标准 → 坐标 → 渲染
```
固定的是第 2 层标准。可演进的是第 1 层编排机制和第 2 层的具体样式参数。
