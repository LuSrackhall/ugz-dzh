...
| 第 2 层 | **正/反页内部层** | 每张 A4 纸正页或反页内部各区域的样式标准 | **本宪法** | "标题深绿 #006100、14pt 加粗" |
...
```
数据流需求 → 第 1 层（Sheet 编排）→ 编排坐标 → 第 2 层（区域样式标准）→ 渲染
```
...
| 数据区（Body） | 分录数行、过次页行、承前页行 | 所有页同行高、同字体 |
...
Step 2: 第 1 层变更 → 调整 LayoutSpec 中 Sheet 编排参数（并排/上下、装订边、列间隙）。
Step 3: 第 2 层变更 → 调整 LayoutSpec 中页内样式参数（标题字体/颜色、列宽比例、行高）。
