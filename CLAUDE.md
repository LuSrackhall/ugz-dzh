...
| 层次 | 名称 | 职责 | 管辖 | 示例 |
|---|---|---|---|---|
| 第 1 层 | **Sheet 编排层** | 正页/反页在 Excel Sheet 中的排布方式（并排/上下、装订边、列间隙、数据流方向） | LayoutEngine | "GL 左半 sheet 是正面，右半是反面" |
| 第 2 层 | **正/反页内部层** | 每张 A4 纸正页或反页内部各区域的样式标准，含**每页数据行数** | **本宪法** | "标题深绿 #006100、14pt 加粗；每页 20 行数据" |
...
**`每页行数` 属于第 2 层**——它定义一张物理页数据区能容纳多少行分录。第 1 层编排时**读取**第 2 层的 `每页行数` 来决定何时过次页，但**不拥有**这个参数。

```
第 2 层定义：每页行数、列宽比例、标题字体/颜色
     │
     ▼ 读取每页行数
第 1 层编排：并排/上下、装订边、列间隙
     │
     ▼ 提供 Layout
Renderer 按坐标渲染
```

第 1 层负责"数据放在 Sheet 的哪里"，第 2 层负责"放上去的时候长什么样"。互不依赖，通过 Layout 协作。
...
| 数据区（Body） | 分录行、过次页行、承前页行 | 所有页同行高、同字体、同 `每页行数` |
...
Step 2: 第 1 层变更 → 调整 LayoutSpec 中 Sheet 编排参数（并排/上下、装订边、列间隙）。读取第 2 层的 `每页行数` 用于编排。
