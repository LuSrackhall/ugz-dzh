...
期初调整额的生效规则：
- **手动调整科目**：在 `生效月` 当月生效——该月期初 = 调整额，账页插入摘要"期初余额"的行；**生效月之后的月份期初 = 上月期末**（余额链连续，调整额不反复覆盖）。
- **自动识别科目**：在 `首次月份` 当月生效（调整额 ≠ 0 时）；首月净额不会成为期初。
- **跨年**：新年度首月期初 = 上年 12 月期末，调整额不会覆盖上年末余额。
- **中途补记**（生效月早于当前月）：历史月份 xlsx 不可修改，请用 `ledger generate -f` 自生效月起级联重建。
