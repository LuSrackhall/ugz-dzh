...
| `platforms.<平台>.fonts.labelFamily` | **摘要/借方/贷方/余额 表头**字体（含"摘要/借..方/贷..方/余..额"文字行 + 金额位数标签行；空=现状宋体；Win 默认"等线 Light"） | Win 等线 Light / Mac 空 |
...
系数用于**粗调**（金额列会集体跳 1px，粗调时无所谓）；精确微调用下面的 **最终 px 增量**（在系数之后叠加，不被缩放；`+1` = 最终 +1px；`0` = 不动）：

| 字段 | 作用列 |
|---|---|
| `frontSummaryDelta` / `backSummaryDelta` | 摘要列（GL 正反面列7/列21，拆4格总量÷4；ML 仅反面列6） |
| `frontBindingDelta` / `backBindingDelta` | **装订边**列（靠装订线那列：GL 列1/列30，ML 列16/列17） |
| `frontOuterDelta` / `backOuterDelta` | **非装订边（书口）**列（GL 列15/列16，ML 列1/列29） |

```json
"ml": { "backSummaryDelta": 5, "backBindingDelta": -2 }
```

- 每个边**只作用一列**，另一列不动
- **金额拆位列完全不参与**（保证组内等宽观感不受影响）
...
`generate` 每次都同时产出两份打印版，互不影响：

| 目录 | 内容 | 用途 |
|---|---|---|
| `print/{月}.xlsx` | **完整打印版**：账页（GL/多科目明细账/分离明细账）+ 日记账 + 报表 + 期初期末表 | 整体打印/查阅 |
| `pdf/{月}.xlsx` | **纯账页版**：仅 总分类账/多科目明细账/分离明细账（位格与打印版一致） | **导出 PDF 成册**：WPS/Excel 打开该文件 → 输出为 PDF，得到无报表混排的纯账本 PDF |

- 两份均为派生产物（xlsx 不进 git），重新 `generate` 刷新；将来 PDF 输出能力亦落此目录；
- v0.10.0–v0.10.2 的 `ledgerOnly` 配置开关**已废弃**（它会改变打印版本体），改为独立目录默认产出——打印版始终完整；配置里若仍写着该字段，generate 会**明确告警**提示可删除（v0.10.4 起）。
