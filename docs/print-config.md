...
以下四种情况都会导致"配置不起作用"，**前三种代码已防呆（会明确报错或自动容忍），但正确用法请照做**：

| # | 错误做法 | 后果 | 正确做法 |
|---|---|---|---|
| 1 | **不传 `--config`**，以为配置放当前目录会自动加载 | 用内置默认值，配置完全无效 | **必须显式传 `--config print-config.json`**（CLI 不会自动查找） |
...
**快速自检**：`generate` 启动时会打印一行 `打印版配置: 默认/已加载 → 平台=.. 列宽系数=.. 行高系数=.. 字体=..`——
- 传了 `--config` 且格式正确 → 显示 `已加载: <路径>` + 你配置的系数值
- 没传 → 显示 `默认`（此时你的配置文件没被读取）
