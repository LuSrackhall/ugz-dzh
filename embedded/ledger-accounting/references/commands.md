...
## voucher — 凭证录入与检查（add / list / check）
```
./ledger voucher add -o <输出根> --date 2026-03-05 --summary "付养老金" \
    --debit "公益支出-补助费用=9990.00" --credit "应付款-养老金=9990.00"
./ledger voucher list  -v <月目录> [--json]
./ledger voucher check -v <月目录> [--strict] [--json]
```
- **`add` 是唯一创建凭证的命令**（此前 CLI 只读凭证）。写入目录由 `--date` 决定：`<输出根>/vouchers/YYYY_MM/`，不可指定其它目录
- **落盘前三道闸门（任一不过即零写盘）**：① 借贷平衡（净额口径，红字为负值参与）；② 科目必须在科目树已定义（复用"先定义后生成"同一套检查，**无 `--allow-new` 逃生**，未定义先 `subjects import` 或 `map` 纠错）；③ 可解析性——写入后立即用解析器往返自检，不一致即删文件报错
- 分录两种形态（互斥）：重复 flag `--debit/--credit "科目=金额"`，或 `--json <文件|->` 吃整张凭证（agent/程序化推荐；JSON 里 `side` 可用 `debit/credit` 或 `借/贷`，`amount` 可用字符串或数字）
- 凭证号：默认自动取当月最大号 +1；`--num` 显式指定（补录用），冲突即拒绝
- `--idempotency-key <k>`：重试幂等——同键重复调用返回首次结果、不产生第二张凭证（索引在 `vouchers/.voucher-keys/YYYY_MM.json`，**在月目录之外**）。索引存在但凭证被人工删除 → 自动重建
- **`add` 不生成账本**：账本生成是用户的显式动作（`generate`）。已生成月份的凭证原则上只冲销；`-f` 改凭证需再三确认
- 写前会跑一次序列/目录卫生检查并**只告警不阻断**（兼容历史账套）；已结账月份会提示需 `-f` 级联重建
- **`check` 三类问题**：缺号（1..max 缺失）/ 重号（同号多文件）/ 非正式文件名或位置（月目录内除 `记字第XXXX号.md` 之外的任何 `.md`）。**默认只告警（退出码 0），`--strict` 才非零退出**——历史账套可能存在作废留空号或带后缀文件名，默认阻断会让它们无法继续生成
- **为什么"非正式文件名"是硬伤害**：凭证收集走 `filepath.WalkDir` **递归**（`cmd/common.go`），月目录内任何 `.md` 都会被解析记账；而重号检测只认文件名里的"记字第"。于是 `草稿.md`/`模板.md` 会被**静默**计入账本，连告警都没有；`记字第0005号 副本.md` 至少还有一条重号告警
- `list` 只列符合正式命名且在月目录根层的凭证（凭证号/日期/摘要/借贷合计/行数）；`check`/`list` 均为只读

## generate — 生成月度账本
```
./ledger generate -v <凭证目录> -o <输出目录> [-f] [-V] [-p <平台>] [--config <print-config.json>] [--allow-new]
```
...
- **先定义后生成（铁律）**：凭证科目（经 map 归并后）必须已在科目树中定义；未定义科目 → 输出清单（科目/出现次数/样例摘要）并**拒绝生成**。处理：`subjects scan`（或旧账转录审核表）→ 确认方向 → `subjects import` → 重跑；OCR 错名先 `map`。**`--allow-new` 为显式逃生**（自动登记新科目，属性=官方表匹配或未分类；临时场景用，日常勿用——未分类科目不进报表、不参与年结转）

生成的账本 xlsx 包含：
- 总分类账-<科目>（每科目一页，三栏式 + 本月合计/本年累计/期末余额）
- 明细账-<总账>-<明细>（若配置"分离明细账科目"：总账名 → 其下所有明细各自分离成页；ML 样式多栏式账页、明细列自然为空，ML 同族月结；与合并 ML 互不冲突共存——合并页明细列照常保留）
- 多科目明细账-<总账科目>（明细多栏；明细列净额以科目基准方向为正——贷方向科目=贷−借，反向发生额打印红字，见 workflows.md）
- 总分类账-<父级>（若配置"合并总账科目"）
- 现金日记账 / 银行存款日记账（逐日逐笔 + 本日合计 + 本月合计/本年累计/期末结存）
- <月>期初 / <月>期末（借贷分列试算平衡）
- 资产负债表 / 收支结余表 / 科目汇总表 / 科目余额表（期初+本月发生+期末一体，对齐旧财务软件导出排版） / 凭证序时簿
- print/ 子目录：**完整打印版**（账页 + 日记账 + 报表 + 期初期末表；GL/ML 拆位 + 红字红色字体）
- pdf/ 子目录：**纯账页版**（仅 GL/多科目明细账/分离明细账，位格同 print/）——供导出 PDF 成册（WPS 打开→输出为 PDF），默认产出
...
## subjects — 科目批量登记（scan / list / export / import）
```
./ledger subjects scan   -v <凭证目录> -j <json> [-o <候选.csv>]
...
- `scan`：扫凭证目录（递归 *.md，**宽容解析**——不要求凭证号/借贷平衡，能吃原始 OCR md）提取出现的科目对与出现频率，生成候选审核表（科目/方向/期初余额/备注/出现次数/样例凭证/状态）。**权威基底原则：审核表基底=旧账期末科目余额表逐行转录，scan 产物仅并入做双向 diff**（scan 看不见"有余额但无发生"的存量科目——固定资产/累计折旧/资本等期初最重科目往往不出现在凭证里）；「期初余额」一律留空（金额来自旧账，scan 不造数）。方向建议：已登记沿用原属性 → 官方科目表（财会〔2023〕14号 42 科目）属性 → 留空待确认。产物可直接回喂 `subjects import`（确认/补全空方向后）
- `import`：批量登记科目并设置属性。新科目以 0 期初登记；**已存在科目只更新属性，绝不动期初调整额**
- 未分类科目指定属性的唯一入口（属性=方向）
- `export`：导出科目树为审核表格式，供与旧账科目余额表做双向 diff（盘点）
- 合并总账科目（父级本身）禁止登记；其子科目正常登记
...
```
- **两段齐发**（默认）：① 一段损益结转——仍有余额的收入/费用科目 → `closing/记字第X号 年末损益结转.md`（结转至 本年收益）；② 二段收益结转——本年收益→**收益分配-未分配收益**（净收益：借 本年收益/贷 收益分配；净亏损方向相反）→ `closing/记字第X号 年末收益结转.md`
- `--no-transfer` 跳过二段（仅一段；下次不带 flag 重跑 gen-close 会按"期初+全部损益发生"重算补发二段）
- 幂等：一段按"科目出现过即跳过"；二段独立口径——closing/ 已有含收益分配分录的凭证即跳过
- 自动登记结转目标科目（本年收益、收益分配-未分配收益，属性=贷）——先定义后生成要求其存在于科目树
- 凭证日期 = 余额月最后一天；借贷恒平衡；closing/ 按年隔离
- closing/ 是派生产物（不进 git）；重新 generate 该月即完成年末结转（损益与本年收益归零，净额挂账收益分配-未分配收益）
...
- 未知类别科目提示（属性=未分类）+ **统计行**（N 个 + 最新月余额合计）——逐科目带余额；未分类科目不进报表合计、不参与年结转，余额非 0 连续 3 个月未处理或跨年带入即科目体系事故，须及时 `subjects import` 补属性或 `map` 纠名
