...
- **纳入 git 管理**：账本目录若为 git 仓库，安装/升级技能后执行 `git add -f .agents/skills/ && git commit`（`-f` 因 .gitignore 默认忽略安装产物；若仓库未忽略则普通 add 即可），技能版本随仓库可追踪、可回滚、可随仓库分发（换机 clone 后重跑 install-skill 即恢复）
