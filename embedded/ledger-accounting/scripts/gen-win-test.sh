...
#
# ⚠️ 本脚本是【开发环境（Mac，含源码）】辅助工具，生产环境（Windows，仅 exe+skill）不需要它——
#    生产环境直接用: ledger generate -v <凭证目录> -o <输出目录> --platform windows
#    打印版尺寸调整用 print-config.json（见 references/print-config.md），改完重新生成即可。
#
