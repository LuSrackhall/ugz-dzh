...
func GenerateWorkbook(configPath, month, outputDir string, entries []voucher.Entry) error {
	...
	// 9.03 报表套件（Change 11：资产负债表/收支结余表/科目汇总表/科目余额表/凭证序时簿）
	if err := wb.WriteReportSheets(entries, activity, initials, ytdDebit, ytdCredit); err != nil {
		return fmt.Errorf("生成报表: %w", err)
	}
	...
	// 9.2 注：**不调用 FinalizeMLPages**（历史方案，af47835 已移除）。续写定位
	// 自 af47835 起用"月度尾行标记列"（期末余额行写月份标识），不依赖页尾占位内容；
	// 若整体调用 FinalizeMLPages 会对每张 ML/分离页补出全空页尾占位块（凭空多页，下游实测）。
	// 当月有变动页的页尾结构过次页仍由月结末尾的 padMLPage 补齐（见 WriteMLMonthClosings）。
