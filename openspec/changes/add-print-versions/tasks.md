...
- [x] 4.1 修改 `cmd/generate.go`，添加 `-view-only`、`-print-only`、`-html-only` 参数
- [x] 4.2 实现按需生成逻辑，根据参数调用不同的生成函数
- [x] 4.3 实现默认行为：无参数时生成所有三套输出
- [x] 4.4 更新帮助文档，说明新参数的用法

## 5. 集成测试

- [x] 5.1 编写端到端测试，验证三套输出的数据一致性
- [x] 5.2 测试 3 个月凭证的完整生成流程
- [x] 5.3 验证打印版 Excel 的列宽和打印效果
- [x] 5.4 验证 HTML 版的浏览器打印效果
- [x] 5.5 更新 README.md，说明新的打印功能和用法
