...
- **表格只写真实分录行，不做空行填充**（实施期订正）：设计初稿写的是"填满固定行数"，但抽样既有凭证（`test/e2e/test_data/2026_04/`）发现填充行数为 **0–3 不等**，是 OCR 原件版式的产物而非约定，无固定值可对齐；解析器本就跳过空行，故不作为生成规则。要改可后续加 `--pad N`。
...
## D7 token 与鉴权（2026-09-13 修订：原"拒绝绑定"改为"自动生成"）

- **token 不是大模型的 token**，是访问口令。绑定局域网后 MCP 端点就是普通 HTTP 网址，同 Wi-Fi 上任何设备都能访问；无鉴权则任何人都可读走全部账目（`ledger.query` / `voucher.list`）或插入假凭证（`voucher.add`）。需要鉴权的分界是**跨设备**，不是"局域网"本身——本机回环免 token。
- token 来源优先级：`--token` flag > 环境变量 `LEDGER_MCP_TOKEN` > 账套配置文件。
- **首次绑定非回环地址且未配置 token 时：自动生成随机 token → 写入账套配置文件（git 可见可改）→ 启动时打印。** 不拒绝启动。
  - 修订理由：原设计担心"用户以为有鉴权、实际是默认值"，但**随机生成 + 落盘 + 打印**已消除该风险（不存在固定默认密码），同时免去第一次接入多敲一条命令的摩擦。要禁止的是**固定默认值**，不是自动生成。
- 无鉴权只能显式开启：`--insecure`（默认关闭）。未传 `--insecure` 时，任何非回环绑定都必须有有效 token（配置 / 环境 / flag / 自动生成四者之一）。
- 接入便利：启动打印账套路径 + 局域网 URL + token，可选二维码；mDNS 自动发现本期不做（Open Questions）。
- **诚实边界**：局域网阶段走明文 HTTP，token 在网内明文传输；威胁模型是"同 Wi-Fi 的普通设备"（访客、邻居手机），该级别 token 足够。防专业抓包需 TLS 自签证书，会带来客户端信任摩擦，留待外网联网阶段——届时要求只增不减（token 必须 + TLS），故现在做 token 不算提前投入。
